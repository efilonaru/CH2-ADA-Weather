    ...
    var body: some View {
        HStack {
            ...
            VStack(spacing: 12) {
                if isMenuExpanded {
                    // Settings Button
                    GameActionButton(systemName: "gearshape.fill", tintColor: .black) {
                        viewModel.showSettings = true
                        withAnimation { isMenuExpanded = false }
                    }
                    .transition(.move(edge: .bottom).combined(with: .opacity))
                    ...
                    GameActionButton(
                        systemName: "pencil",
                        tintColor: viewModel.isEditMode ? .purple : .black
                    ) {
                        viewModel.toggleEditMode()
                        withAnimation { isMenuExpanded = false }
                    }
                    ...
                    // Inventory Button
                    GameActionButton(systemName: "bag.fill", tintColor: .black) {
                        viewModel.showInventory = true
                        withAnimation { isMenuExpanded = false }
                    }
                    .transition(.move(edge: .bottom).combined(with: .opacity))
                    ...
                    // Shop Button
                    GameActionButton(systemName: "cart.fill", tintColor: .black) {
                        viewModel.showShop = true
                        withAnimation { isMenuExpanded = false }
                    }
                    .transition(.move(edge: .bottom).combined(with: .opacity))
